...
def find_or_create_private_chat(user1, user2):
	try:
		chat = PrivateChatRoom.objects.get((Q(user1=user1) & Q(user2=user2)) | (Q(user2=user1) & Q(user1=user2)), is_active=True)
...
class LazyRoomChatMessageEncoder(Serializer):
    def get_dump_object(self, obj):
        ...
        dump_object.update({'profile_pic': str(obj.user.profile_pic.url)})
